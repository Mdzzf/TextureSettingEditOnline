$(document).ready(function () {
    ...
    $("#convert").click(function(){
        var $this=$("#xmlConfig").val();
        var $old=$("#oldText").val();
        if(!isEmpty($this)&&($this!=$old)){
            run();
        }else{
            alert("can't convert null xml or same xml config.")
        }
    })
});
...
function run(){
    ...
    var $xml=$this.val();
    var data=[];
    $($xml).find('ExcludeFoldersGobal').each(function(){
        var li=$(this).children('li');
        if(!isEmpty(li)&&li.length>0){
            for(var i=0;i<li.length;i++){
                var text=$(li[i]).text().trim();
                if(!isEmpty(text)){
                    data.push({
                        Folder:text
                    })
        ...
        $("#excludeFoldersTable").bootstrapTable('load',data);
    });
    data=[];
    $($xml).find('AppendGobal').each(function(){
        var li=$(this).children('li');
        if(!isEmpty(li)&&li.length>0){
            for(var i=0;i<li.length;i++){
                var text=$(li[i]).text().trim();
        ...
        $("#appendAlienBodyTypesTable").bootstrapTable('load',data)
    });
    data=[];
    $($xml).find('packages').each(function(){
        var package=$(this).children('package');
        if(!isEmpty(package)&&package.length>0){
            for(var i=0;i<package.length;i++){
                var $packageId=$($(package[i]).children('packageId')).text();
                if(!isEmpty($packageId)){
                    var $excludeFolders=$(package[i]).children('excludeFolders').children('li');
                    var excludeFoldersStr="";
                    if(!isEmpty($excludeFolders)&&$excludeFolders.length>0){
                        for(var j=0;j<$excludeFolders.length;j++){
                            var text=$($excludeFolders[j]).text().trim();
                            if(!isEmpty(text)){
                                excludeFoldersStr+=text+";";
                            }
                        }
                        excludeFoldersStr=excludeFoldersStr.substr(0,excludeFoldersStr.length-1);
                    }
                    data.push({
                        PackageId:$packageId,
                        ExcludeFolders:excludeFoldersStr
                    })

                }
            }
        }
        $("#packageTable").bootstrapTable('load',data);
    });
}
